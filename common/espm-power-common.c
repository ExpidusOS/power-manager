/*
 * * Copyright (C) 2010-2011 Ali <aliov@expidus.org>
 *
...
/**
 * espm_power_translate_device_type:
 *
 **/
const gchar *
espm_power_translate_device_type (guint type)
{
  switch (type)
  {
...
/**
 * espm_power_translate_technology:
 *
 **/
const gchar *
espm_power_translate_technology (guint value)
{
  switch (value)
  {
    case 0:
...
static const gchar *
espm_battery_get_icon_index (guint percent)
{
  if (percent < 10)
    return "0";
  if (percent < 20)
 ...
 */
gchar *
espm_battery_get_time_string (guint seconds)
{
...
get_device_icon_name (UpClient *upower, UpDevice *device, gboolean is_panel)
{
  ...
  ESPM_DEBUG ("icon_suffix %s, icon_base_length %ld, upower_icon %s",
  ...
  if ( type == UP_DEVICE_KIND_BATTERY && is_panel )
  {
    if ( state == UP_DEVICE_STATE_CHARGING || state == UP_DEVICE_STATE_PENDING_CHARGE)
      icon_name = g_strdup_printf ("%s-%s-%s", ESPM_BATTERY_LEVEL_ICON, espm_battery_get_icon_index (percentage), "charging-symbolic");
    else if ( state == UP_DEVICE_STATE_DISCHARGING || state == UP_DEVICE_STATE_PENDING_DISCHARGE)
      icon_name = g_strdup_printf ("%s-%s-%s", ESPM_BATTERY_LEVEL_ICON, espm_battery_get_icon_index (percentage), "symbolic");
    else if ( state == UP_DEVICE_STATE_FULLY_CHARGED)
      icon_name = g_strdup_printf ("%s-%s", ESPM_BATTERY_LEVEL_ICON, "100-charged-symbolic");
    else
      icon_name = g_strdup ("battery-missing-symbolic");
  }
  else if ( type == UP_DEVICE_KIND_UPS )
    icon_name = g_strdup (ESPM_UPS_ICON);
  else if ( type == UP_DEVICE_KIND_MOUSE )
    icon_name = g_strdup (ESPM_MOUSE_ICON);
  else if ( type == UP_DEVICE_KIND_KEYBOARD )
    icon_name = g_strdup (ESPM_KBD_ICON);
  else if ( type == UP_DEVICE_KIND_PHONE )
    icon_name = g_strdup (ESPM_PHONE_ICON);
  else if ( type == UP_DEVICE_KIND_PDA )
    icon_name = g_strdup (ESPM_PDA_ICON);
  else if ( type == UP_DEVICE_KIND_MEDIA_PLAYER )
    icon_name = g_strdup (ESPM_MEDIA_PLAYER_ICON);
  else if ( type == UP_DEVICE_KIND_LINE_POWER )
    icon_name = g_strdup_printf ("%s%s", ESPM_AC_ADAPTER_ICON, is_panel ? "-symbolic" : "");
  else if ( type == UP_DEVICE_KIND_MONITOR )
    icon_name = g_strdup (ESPM_MONITOR_ICON);
  else if ( type == UP_DEVICE_KIND_TABLET )
    icon_name = g_strdup (ESPM_TABLET_ICON);
  else if ( type == UP_DEVICE_KIND_COMPUTER )
    icon_name = g_strdup (ESPM_COMPUTER_ICON);
...
get_device_description (UpClient *upower, UpDevice *device)
{
  ...
  if (g_strcmp0(vendor, "") == 0 && g_strcmp0(model, "") == 0)
    vendor = g_strdup_printf ("%s", espm_power_translate_device_type (type));
  ...
  if ( state == UP_DEVICE_STATE_FULLY_CHARGED )
  {
    if ( time_to_empty > 0 )
    {
      est_time_str = espm_battery_get_time_string (time_to_empty);
  ...
  else if ( state == UP_DEVICE_STATE_CHARGING )
  {
    if ( time_to_full != 0 )
    {
      est_time_str = espm_battery_get_time_string (time_to_full);
      tip = g_strdup_printf (_("<b>%s %s</b>\nCharging (%0.0f%%, %s)"),
                             vendor, model,
                             percentage,
                             est_time_str);
