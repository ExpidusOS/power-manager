...
static void espm_console_kit_finalize     (GObject *object);

static void espm_console_kit_get_property (GObject *object,
                                           guint prop_id,
                                           GValue *value,
                                           GParamSpec *pspec);

struct EspmConsoleKitPrivate
{
  GDBusConnection *bus;
  GDBusProxy      *proxy;
...
G_DEFINE_TYPE_WITH_PRIVATE (EspmConsoleKit, espm_console_kit, G_TYPE_OBJECT)

static void
espm_console_kit_get_info (EspmConsoleKit *console)
{
  GError *error = NULL;
  gchar *tmp = NULL;
  GVariant *var;
...
static void
espm_console_kit_class_init (EspmConsoleKitClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = espm_console_kit_finalize;

  object_class->get_property = espm_console_kit_get_property;
...
static void
espm_console_kit_init (EspmConsoleKit *console)
{
  GError *error = NULL;

  console->priv = espm_console_kit_get_instance_private (console);
  ...
  espm_console_kit_get_info (console);
}
             ...
             GParamSpec *pspec)
{
  ...
  console = ESPM_CONSOLE_KIT (object);
...
static void
espm_console_kit_finalize (GObject *object)
{
  EspmConsoleKit *console;
  ...
  G_OBJECT_CLASS (espm_console_kit_parent_class)->finalize (object);
}

EspmConsoleKit *
espm_console_kit_new (void)
{
  ...
  {
    console_obj = g_object_new (ESPM_TYPE_CONSOLE_KIT, NULL);
    g_object_add_weak_pointer (console_obj, &console_obj);
  }

  return ESPM_CONSOLE_KIT (console_obj);
}

void espm_console_kit_shutdown (EspmConsoleKit *console, GError **error)
{
  GVariant *var;
...
void espm_console_kit_reboot (EspmConsoleKit *console, GError **error)
{
  GVariant *var;
...
void
espm_console_kit_suspend (EspmConsoleKit *console,
                          GError        **error)
{
  GVariant *var;
...
void
espm_console_kit_hibernate (EspmConsoleKit *console,
                            GError        **error)
{
  GVariant *var;
