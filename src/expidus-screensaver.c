...
#define XFSM_CHANNEL            "expidus1-session"
...
struct ExpidusScreenSaverPrivate
{
  ...
  EsconfChannel   *espm_channel;
  EsconfChannel   *xfsm_channel;
};


G_DEFINE_TYPE_WITH_PRIVATE (ExpidusScreenSaver, expidus_screensaver, G_TYPE_OBJECT)
...
expidus_screensaver_init (ExpidusScreenSaver *saver)
{
  ...
  saver->priv = expidus_screensaver_get_instance_private (saver);
  ...
  {
          saver->priv->espm_channel = esconf_channel_get (ESPM_CHANNEL);
          saver->priv->xfsm_channel = esconf_channel_get (XFSM_CHANNEL);
  }
...
expidus_screensaver_new (void)
{
  ...
  else
  {
    ...
    esconf_g_property_bind (EXPIDUS_SCREENSAVER (saver)->priv->espm_channel,
                            ESPM_PROPERTIES_PREFIX HEARTBEAT_COMMAND,
                            G_TYPE_STRING,
                            G_OBJECT(saver),
                            HEARTBEAT_COMMAND);

    esconf_g_property_bind (EXPIDUS_SCREENSAVER (saver)->priv->xfsm_channel,
                          ...
                          gboolean inhibit)
{
   ...
   * SCREENSAVER_TYPE_GNOME and SCREENSAVER_TYPE_EXPIDUS
