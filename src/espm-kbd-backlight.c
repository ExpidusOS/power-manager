...
static void espm_kbd_backlight_finalize     (GObject *object);

struct EspmKbdBacklightPrivate
{
  EspmPower          *power;
  EspmButton         *button;
...
G_DEFINE_TYPE_WITH_PRIVATE (EspmKbdBacklight, espm_kbd_backlight, G_TYPE_OBJECT)
...
static void
espm_kbd_backlight_on_battery_changed_cb (EspmPower *power, gboolean on_battery, EspmKbdBacklight *backlight)
{
...
static void
espm_kbd_backlight_init_max_level (EspmKbdBacklight *backlight)
{
  GError *error = NULL;
  GVariant *var;
...
static void
espm_kbd_backlight_show_notification (EspmKbdBacklight *self, gfloat value)
{
  gchar *summary;
  ...
  if ( self->priv->n == NULL )
  {
    ...
    self->priv->n = espm_notify_new_notification (self->priv->notify,
                                                  _("Power Manager"),
                                                  summary,
                                                  "keyboard-brightness",
                                                  ESPM_NOTIFY_NORMAL);
...
static gint
espm_kbd_backlight_get_level (EspmKbdBacklight *backlight)
{
  GError *error = NULL;
  gint32 level = -1;
  GVariant *var;
...
static void
espm_kbd_backlight_set_level (EspmKbdBacklight *backlight, gint32 level)
{
  GError *error = NULL;
  gfloat percent;
  GVariant *var;
  ...
  else
  {
    ...
    espm_kbd_backlight_show_notification (backlight, percent);
  }
}

static void
espm_kbd_backlight_up (EspmKbdBacklight *backlight)
{
  gint level;

  level = espm_kbd_backlight_get_level (backlight);
  ...
  espm_kbd_backlight_set_level (backlight, level);
}


static void
espm_kbd_backlight_down (EspmKbdBacklight *backlight)
{
  gint level;
...
static void
espm_kbd_backlight_button_pressed_cb (EspmButton *button, EspmButtonKey type, EspmKbdBacklight *backlight)
{
  if ( type == BUTTON_KBD_BRIGHTNESS_UP )
  {
    espm_kbd_backlight_up (backlight);
  }
  else if ( type == BUTTON_KBD_BRIGHTNESS_DOWN )
  {
    espm_kbd_backlight_down (backlight);
  }
}


static void
espm_kbd_backlight_class_init (EspmKbdBacklightClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = espm_kbd_backlight_finalize;
}


static void
espm_kbd_backlight_init (EspmKbdBacklight *backlight)
{
  GError *error = NULL;

  backlight->priv = espm_kbd_backlight_get_instance_private (backlight);
  ...
  espm_kbd_backlight_init_max_level (backlight);
  ...
  g_signal_connect (backlight->priv->button, "button-pressed",
                    G_CALLBACK (espm_kbd_backlight_button_pressed_cb), backlight);

  g_signal_connect (backlight->priv->power, "on-battery-changed",
                    G_CALLBACK (espm_kbd_backlight_on_battery_changed_cb), backlight);
...
static void
espm_kbd_backlight_finalize (GObject *object)
{
  EspmKbdBacklight *backlight = NULL;

  backlight = ESPM_KBD_BACKLIGHT (object);
  ...
  G_OBJECT_CLASS (espm_kbd_backlight_parent_class)->finalize (object);
}


EspmKbdBacklight *
espm_kbd_backlight_new (void)
{
  EspmKbdBacklight *backlight = NULL;
  backlight = g_object_new (ESPM_TYPE_KBD_BACKLIGHT, NULL);
  return backlight;
}
