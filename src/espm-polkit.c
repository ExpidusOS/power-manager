...
static void espm_polkit_finalize   (GObject *object);

struct EspmPolkitPrivate
{
  GDBusConnection   *bus;
...
G_DEFINE_TYPE_WITH_PRIVATE (EspmPolkit, espm_polkit, G_TYPE_OBJECT)
...
#ifdef ENABLE_POLKIT
static void
espm_polkit_free_data (EspmPolkit *polkit)
{
  g_assert (polkit->priv->subject_valid);

  ESPM_DEBUG ("Destroying Polkit data");
...
static gboolean
espm_polkit_free_data_cb (gpointer user_data)
{
  EspmPolkit *polkit;

  polkit = ESPM_POLKIT (user_data);
  espm_polkit_free_data (polkit);

  return FALSE;
}

static void
espm_polkit_init_data (EspmPolkit *polkit)
{
  gint pid;
  guint64 start_time;
  ...
  if ( G_LIKELY (start_time != 0 ) )
  {
    ...
    ESPM_DEBUG ("Using unix session polkit subject");
  }
  else
  {
  ...
  polkit->priv->destroy_id = g_timeout_add_seconds (60, espm_polkit_free_data_cb, polkit);
...
static gboolean
espm_polkit_check_auth_intern (EspmPolkit *polkit, const gchar *action_id)
{
#ifdef ENABLE_POLKIT
  GError *error = NULL;
  ...
  ESPM_DEBUG ("polkit request: %s", g_variant_print (var, TRUE));
  ...
  ESPM_DEBUG ("Action=%s is authorized=%s", action_id, espm_bool_to_string (is_authorized));
...
#ifdef ENABLE_POLKIT
static void
espm_polkit_changed_cb (GDBusProxy *proxy, EspmPolkit *polkit)
{
  ESPM_DEBUG ("Auth changed");
  g_signal_emit (G_OBJECT (polkit), signals [AUTH_CHANGED], 0);
}
#endif

static void
espm_polkit_class_init (EspmPolkitClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = espm_polkit_finalize;

  signals [AUTH_CHANGED] =
      g_signal_new ("auth-changed",
                    ESPM_TYPE_POLKIT,
                    G_SIGNAL_RUN_LAST,
                    G_STRUCT_OFFSET(EspmPolkitClass, auth_changed),
                    NULL, NULL,
...
static void
espm_polkit_init (EspmPolkit *polkit)
{
  GError *error = NULL;

  polkit->priv = espm_polkit_get_instance_private (polkit);
  ...
  if (G_LIKELY (polkit->priv->proxy) )
  {
    ...
    G_CALLBACK (espm_polkit_changed_cb), polkit);
  }
  else
  {
...
static void
espm_polkit_finalize (GObject *object)
{
  EspmPolkit *polkit;

  polkit = ESPM_POLKIT (object);
  ...
  G_OBJECT_CLASS (espm_polkit_parent_class)->finalize (object);
}

EspmPolkit *
espm_polkit_get (void)
{
  static gpointer espm_polkit_obj = NULL;

  if ( G_LIKELY (espm_polkit_obj) )
  {
    g_object_ref (espm_polkit_obj);
  }
  else
  {
    espm_polkit_obj = g_object_new (ESPM_TYPE_POLKIT, NULL);
    g_object_add_weak_pointer (espm_polkit_obj, &espm_polkit_obj);
  }

  return ESPM_POLKIT (espm_polkit_obj);
}

gboolean
espm_polkit_check_auth (EspmPolkit *polkit, const gchar *action_id)
{
#ifdef ENABLE_POLKIT
  espm_polkit_init_data (polkit);
#endif
  return espm_polkit_check_auth_intern (polkit, action_id);
}
