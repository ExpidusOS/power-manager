...
gboolean
espm_suspend_can_hibernate (void)
{
  ESPM_DEBUG("entering");
#ifdef BACKEND_TYPE_FREEBSD
...
gboolean
espm_suspend_try_action (EspmActionType type)
{
  const gchar *action;
  gboolean ret;
  GError *error = NULL;
  ...
  if (type == ESPM_SUSPEND)
    action = "suspend";
  else if (type == ESPM_HIBERNATE)
    action = "hibernate";
  else
    return FALSE;

  command = g_strdup_printf ("pkexec " SBINDIR "/expidus1-pm-helper --%s", action);
  ret = g_spawn_command_line_sync (command, NULL, NULL, &exit_status, &error);
  ...
  if ( !ret )
  {
    if (error)
    {
      g_warning ("expidus1-pm-helper: failed to suspend/hibernate: %s", error->message);
      g_error_free (error);
    }
  }
  else
  {
    ESPM_DEBUG ("executed %s; retval: %i", command, exit_status);
    ret = (exit_status == 0);
  }
