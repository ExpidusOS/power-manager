...
#include "interfaces/espm-settings_ui.h"
...
enum
{
  ESPM_DEVICE_INFO_NAME,
  ESPM_DEVICE_INFO_VALUE,
  ESPM_DEVICE_INFO_LAST
};
...
void        light_locker_automatic_locking_changed_cb  (GtkWidget *w,
                                                        EsconfChannel *channel);
void        espm_update_logind_handle_lid_switch       (EsconfChannel *channel);
/* END Light Locker Integration */
...
void brightness_level_on_ac (GtkWidget *w,  EsconfChannel *channel)
{
  guint val = (guint) gtk_range_get_value (GTK_RANGE (w));

  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_LEVEL_ON_AC, val) )
  {
...
void brightness_level_on_battery (GtkWidget *w,  EsconfChannel *channel)
{
   guint val = (guint) gtk_range_get_value (GTK_RANGE (w));

  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_LEVEL_ON_BATTERY, val) )
  {
...
void
battery_critical_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkTreeModel     *model;
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX CRITICAL_BATT_ACTION_CFG, value) )
  {
...
void
inactivity_on_ac_value_changed_cb (GtkWidget *widget, EsconfChannel *channel)
{
  gint value    = (gint)gtk_range_get_value (GTK_RANGE (widget));

  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX ON_AC_INACTIVITY_TIMEOUT, value))
  {
...
void
inactivity_on_battery_value_changed_cb (GtkWidget *widget, EsconfChannel *channel)
{
  gint value    = (gint)gtk_range_get_value (GTK_RANGE (widget));

  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX ON_BATTERY_INACTIVITY_TIMEOUT, value))
  {
...
void
button_sleep_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkTreeModel     *model;
  GtkTreeIter       selected_row;
  gint value = 0;
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX SLEEP_SWITCH_CFG, value ) )
  {
    g_critical ("Cannot set value for property %s\n", SLEEP_SWITCH_CFG);
  }
}
...
void
button_power_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkTreeModel     *model;
  GtkTreeIter       selected_row;
  gint value = 0;
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX POWER_SWITCH_CFG, value) )
  {
    g_critical ("Cannot set value for property %s\n", POWER_SWITCH_CFG);
  }
}
...
void
button_hibernate_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkTreeModel     *model;
  GtkTreeIter       selected_row;
  gint value = 0;
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX HIBERNATE_SWITCH_CFG, value ) )
  {
...
void
button_battery_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkTreeModel     *model;
  GtkTreeIter       selected_row;
  gint value = 0;
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX BATTERY_SWITCH_CFG, value ) )
  {
    g_critical ("Cannot set value for property %s\n", BATTERY_SWITCH_CFG);
  }
}
...
void
on_ac_sleep_mode_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkTreeModel     *model;
  GtkTreeIter       selected_row;
  guint value = 0;
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX INACTIVITY_SLEEP_MODE_ON_AC, value) )
  {
...
void
on_battery_sleep_mode_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkTreeModel     *model;
  GtkTreeIter       selected_row;
  guint value = 0;
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX INACTIVITY_SLEEP_MODE_ON_BATTERY, value) )
  {
...
gboolean
dpms_toggled_cb (GtkWidget *w, gboolean is_active, EsconfChannel *channel)
{
  esconf_channel_set_bool (channel, ESPM_PROPERTIES_PREFIX DPMS_ENABLED_CFG, is_active);
...
void
display_blank_on_battery_value_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkWidget *brg;
...
void
sleep_on_battery_value_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkWidget *brg;
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX ON_BATT_DPMS_SLEEP, sleep_value))
  {
    g_critical ("Cannot set value for property %s\n", ON_BATT_DPMS_SLEEP);
  }
...
void
off_on_battery_value_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX ON_BATT_DPMS_OFF, off_value))
  {
    g_critical ("Cannot set value for property %s\n", ON_BATT_DPMS_OFF);
  }
...
void
display_blank_on_ac_value_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkWidget *brg;
...
void
sleep_on_ac_value_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkWidget *brg;
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX ON_AC_DPMS_SLEEP, sleep_value))
  {
    g_critical ("Cannot set value for property %s\n", ON_AC_DPMS_SLEEP);
  }
...
void
off_on_ac_value_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  gint off_value    = (gint)gtk_range_get_value (GTK_RANGE(w));
  gint sleep_value  = (gint)gtk_range_get_value (GTK_RANGE(on_ac_dpms_sleep));
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX ON_AC_DPMS_OFF, off_value))
  {
    g_critical ("Cannot set value for property %s\n", ON_AC_DPMS_OFF);
  }
...
void
brightness_on_battery_value_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_ON_BATTERY, value))
  {
...
void
brightness_on_ac_value_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  gint value    = (gint)gtk_range_get_value (GTK_RANGE (w));
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_ON_AC, value))
  {
    g_critical ("Cannot set value for property %s\n", BRIGHTNESS_ON_AC);
  }
...
void
on_battery_lid_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkTreeModel     *model;
  GtkTreeIter       selected_row;
  gint value = 0;
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX LID_SWITCH_ON_BATTERY_CFG, value) )
  {
    g_critical ("Cannot set value for property %s\n", LID_SWITCH_ON_BATTERY_CFG);
  }
  espm_update_logind_handle_lid_switch (channel);
}

void
on_ac_lid_changed_cb (GtkWidget *w, EsconfChannel *channel)
{
  GtkTreeModel     *model;
  GtkTreeIter       selected_row;
  gint value = 0;
  ...
  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX LID_SWITCH_ON_AC_CFG, value) )
  {
...
void
critical_level_value_changed_cb (GtkSpinButton *w, EsconfChannel *channel)
{
  guint val = (guint) gtk_spin_button_get_value (w);

  if (!esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX CRITICAL_POWER_LEVEL, val) )
  {
...
gboolean
handle_brightness_keys_toggled_cb (GtkWidget *w, gboolean is_active, EsconfChannel *channel)
{
...
void
brightness_step_count_value_changed_cb (GtkSpinButton *w, EsconfChannel *channel)
{
  guint val = (guint) gtk_spin_button_get_value (w);

  if ( !esconf_channel_set_uint (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_STEP_COUNT, val) )
  {
...
void
brightness_exponential_toggled_cb (GtkWidget *w, EsconfChannel *channel)
{
  gboolean val = (gint) gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(w));

  if ( !esconf_channel_set_bool (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_EXPONENTIAL, val) )
  {
...
void
lock_screen_toggled_cb (GtkWidget *w, EsconfChannel *channel)
{
  EsconfChannel *session_channel = esconf_channel_get ("expidus1-session");
  gboolean val = (gint) gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(w));

  if ( !esconf_channel_set_bool (session_channel, "/shutdown/LockScreen", val) )
  {
    g_critical ("Unable to set value for property %s\n", LOCK_SCREEN_ON_SLEEP);
  }

  if ( !esconf_channel_set_bool (channel, ESPM_PROPERTIES_PREFIX LOCK_SCREEN_ON_SLEEP, val) )
  {
    g_critical ("Unable to set value for property %s\n", LOCK_SCREEN_ON_SLEEP);
  }
...
/* Light Locker Integration */
void
espm_update_logind_handle_lid_switch (EsconfChannel *channel)
{
  gboolean lock_on_suspend = esconf_channel_get_bool (channel, ESPM_PROPERTIES_PREFIX LOCK_SCREEN_ON_SLEEP, TRUE);
  guint lid_switch_on_ac = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX LID_SWITCH_ON_AC_CFG, LID_TRIGGER_LOCK_SCREEN);
  guint lid_switch_on_battery = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX LID_SWITCH_ON_BATTERY_CFG, LID_TRIGGER_LOCK_SCREEN);

  // logind-handle-lid-switch = true when: lock_on_suspend == true and (lid_switch_on_ac == suspend and lid_switch_on_battery == suspend)
  esconf_channel_set_bool (channel, ESPM_PROPERTIES_PREFIX LOGIND_HANDLE_LID_SWITCH, lock_on_suspend && (lid_switch_on_ac == 1 && lid_switch_on_battery == 1));
}
/* END Light Locker Integration */

static void
espm_settings_on_battery (EsconfChannel *channel, gboolean auth_suspend,
                          ...
                          gboolean has_lcd_brightness, gboolean has_lid)
{
  ...
  if ( can_suspend )
  {
    ...
    gtk_list_store_set (list_store, &iter, 0, _("Suspend"), 1, ESPM_DO_SUSPEND, -1);
  }
  ...
  {
    gtk_list_store_append (list_store, &iter);
    gtk_list_store_set (list_store, &iter, 0, _("Hibernate"), 1, ESPM_DO_HIBERNATE, -1);
  }
  ...
  val = esconf_channel_get_uint (channel,
                                 ESPM_PROPERTIES_PREFIX INACTIVITY_SLEEP_MODE_ON_BATTERY,
                                 ESPM_DO_HIBERNATE);
  ...
  val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX ON_BATTERY_INACTIVITY_TIMEOUT, 14);
  ...
  gtk_list_store_append(list_store, &iter);
  gtk_list_store_set (list_store, &iter, 0, _("Do nothing"), 1, ESPM_DO_NOTHING, -1);
  ...
  {
    gtk_list_store_append(list_store, &iter);
    gtk_list_store_set (list_store, &iter, 0, _("Shutdown"), 1, ESPM_DO_SHUTDOWN, -1);
  }

  gtk_list_store_append(list_store, &iter);
  gtk_list_store_set (list_store, &iter, 0, _("Ask"), 1, ESPM_ASK, -1);

  val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX CRITICAL_BATT_ACTION_CFG, ESPM_DO_NOTHING);
   ...
   */
  handle_dpms = esconf_channel_get_bool (channel, ESPM_PROPERTIES_PREFIX DPMS_ENABLED_CFG, TRUE);

  val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX ON_BATT_DPMS_SLEEP, 5);
  gtk_range_set_value (GTK_RANGE(on_battery_dpms_sleep), val);
  gtk_widget_set_sensitive (on_battery_dpms_sleep, handle_dpms);

  val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX ON_BATT_DPMS_OFF, 10);
  ...
  if ( has_lid )
  {
    ...
    val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX LID_SWITCH_ON_BATTERY_CFG, LID_TRIGGER_LOCK_SCREEN);
  ...
  {
    val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_ON_BATTERY, 120);
    gtk_range_set_value (GTK_RANGE(brg), val);

    val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_LEVEL_ON_BATTERY, 20);
...
static void
espm_settings_on_ac (EsconfChannel *channel, gboolean auth_suspend,
                     gboolean auth_hibernate, gboolean can_suspend,
                     ...
                     gboolean has_lid)
{
  ...
  val = esconf_channel_get_uint (channel,
                                 ESPM_PROPERTIES_PREFIX INACTIVITY_SLEEP_MODE_ON_AC,
                                 ESPM_DO_SUSPEND);
  ...
  val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX ON_AC_INACTIVITY_TIMEOUT, 14);
  gtk_range_set_value (GTK_RANGE (inact_timeout), val);
  ...
  val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX ON_AC_DPMS_SLEEP, 10);
  gtk_range_set_value (GTK_RANGE (on_ac_dpms_sleep), val);
  gtk_widget_set_sensitive (on_ac_dpms_sleep, handle_dpms);

  val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX ON_AC_DPMS_OFF, 15);
  ...
  if ( has_lid )
  {
    ...
    val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX LID_SWITCH_ON_AC_CFG, LID_TRIGGER_LOCK_SCREEN);
    for ( valid = gtk_tree_model_get_iter_first (GTK_TREE_MODEL (list_store), &iter);
          valid;
          valid = gtk_tree_model_iter_next (GTK_TREE_MODEL (list_store), &iter) )
    {
  ...
  if ( has_lcd_brightness )
  {
    val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_ON_AC, 9);
    gtk_range_set_value (GTK_RANGE(brg), val);

    val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_LEVEL_ON_AC, 80);
    gtk_range_set_value (GTK_RANGE (brg_level), val);
  }
  else
  {
...
static void
espm_settings_general (EsconfChannel *channel, gboolean auth_suspend,
                       gboolean auth_hibernate, gboolean can_suspend,
                       gboolean can_hibernate, gboolean can_shutdown,
                       ...
                       gboolean has_power_button, gboolean has_battery_button)
{
   ...
   */
  val = esconf_channel_get_bool (channel, ESPM_PROPERTIES_PREFIX DPMS_ENABLED_CFG, TRUE);
  ...
  if ( has_power_button )
  {
    ...
    value = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX POWER_SWITCH_CFG, ESPM_DO_NOTHING);
  ...
  if (has_hibernate_button )
  {
    ...
    value = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX HIBERNATE_SWITCH_CFG, ESPM_DO_NOTHING);
  ...
  if ( has_sleep_button )
  {
    ...
    value = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX SLEEP_SWITCH_CFG, ESPM_DO_NOTHING);
    for ( valid = gtk_tree_model_get_iter_first (GTK_TREE_MODEL (list_store), &iter);
          valid;
          valid = gtk_tree_model_iter_next (GTK_TREE_MODEL (list_store), &iter) )
    {
  ...
  if ( has_battery_button )
  {
    ...
    value = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX BATTERY_SWITCH_CFG, ESPM_DO_NOTHING);
    for ( valid = gtk_tree_model_get_iter_first (GTK_TREE_MODEL (list_store), &iter);
          valid;
          valid = gtk_tree_model_iter_next (GTK_TREE_MODEL (list_store), &iter) )
    {
  ...
  val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_STEP_COUNT, 10);
  if ( val > 100 || val < 2)
  {
  ...
  val = esconf_channel_get_bool (channel, ESPM_PROPERTIES_PREFIX BRIGHTNESS_EXPONENTIAL, FALSE);
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON(brightness_exponential), val);

  valid = esconf_channel_get_bool (channel, ESPM_PROPERTIES_PREFIX HANDLE_BRIGHTNESS_KEYS, TRUE);
...
static void
espm_settings_advanced (EsconfChannel *channel, gboolean auth_suspend,
                        gboolean auth_hibernate, gboolean can_suspend,
                        gboolean can_hibernate, gboolean has_battery)
{
  ...
  if ( has_battery )
  {
    ...
    val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX CRITICAL_POWER_LEVEL, 10);
  ...
  val = esconf_channel_get_bool (channel, ESPM_PROPERTIES_PREFIX LOCK_SCREEN_ON_SLEEP, TRUE);
...
void
light_locker_late_locking_value_changed_cb (GtkWidget *widget, EsconfChannel *channel)
{
  GVariant *variant;
  gint      value = (gint)gtk_range_get_value (GTK_RANGE (widget));
...
void
light_locker_automatic_locking_changed_cb (GtkWidget *widget, EsconfChannel *channel)
{
  GVariant *variant;
  gint      value;
...
static void espm_settings_light_locker (EsconfChannel *channel,
                                        ...
                                        gboolean can_suspend, gboolean can_hibernate)
{
  ...
  GVariant              *variant;
  gboolean               late_locking, lock_on_suspend, espm_lock_on_suspend;
  ...
  if (schema != NULL && get_light_locker_path() != NULL)
  {
    ...
    espm_lock_on_suspend = esconf_channel_get_bool (channel, ESPM_PROPERTIES_PREFIX LOCK_SCREEN_ON_SLEEP, TRUE);
    if (lock_on_suspend != espm_lock_on_suspend)
    {
      variant = g_variant_new_boolean (espm_lock_on_suspend);
      if (!g_settings_set_value (light_locker_settings, "lock-on-suspend", variant))
      {
        g_critical ("Cannot set value for property lock-on-suspend\n");
      }
      lock_on_suspend = espm_lock_on_suspend;
    }
  ...
  {
    ESPM_DEBUG ("Schema \"apps.light-locker\" not found. Not configuring Light Locker.");
...
find_device_info_name_in_tree (GtkTreeView *view, const gchar *device_info_name)
{
  ...
  if (gtk_tree_model_get_iter_first (model, &iter)) {
    do {
      gchar *name = NULL;
      gtk_tree_model_get (model, &iter, ESPM_DEVICE_INFO_NAME, &name, -1);
...
static gchar *
espm_info_get_energy_property (gdouble energy, const gchar *unit)
{
  gchar *val = NULL;
                                   ...
                                   const gchar *value)
{
  ...
  if (value != NULL)
  {
    gtk_list_store_set (list_store, iter,
                        ESPM_DEVICE_INFO_NAME, name,
                        ESPM_DEVICE_INFO_VALUE, value,
                        -1);
  }
  else
  {
...
update_device_details (UpDevice *device)
{
  ...
  {
    battery_type = espm_power_translate_device_type (type);
  ...
  if ( type != UP_DEVICE_KIND_LINE_POWER )
  {
    ...
    update_device_info_value_for_name (view, list_store, _("Technology"), espm_power_translate_technology (tech));
    ...
    if (energy_full_design > 0)
    {
      ...
      str = espm_info_get_energy_property (energy_full_design, _("Wh"));
    ...
    if (energy_full > 0)
    {
      ...
      /* TRANSLATORS: Unit here is Watt hour*/
      str = espm_info_get_energy_property (energy_full, _("Wh"));
    ...
    {
      /* TRANSLATORS: Unit here is Watt hour*/
      str = espm_info_get_energy_property (energy_empty, _("Wh"));
    ...
    if (voltage > 0)
    {
      ...
      str = espm_info_get_energy_property (voltage, _("V"));
...
add_device (UpDevice *device)
{
  ...
  devices_store = gtk_list_store_new (ESPM_DEVICE_INFO_LAST, G_TYPE_STRING, G_TYPE_STRING);
  ...
  gtk_tree_view_column_set_attributes (col, renderer, "text", ESPM_DEVICE_INFO_NAME, NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW (view), col);

  /*Device Attribute Value*/
  col = gtk_tree_view_column_new();
  gtk_tree_view_column_pack_start (col, renderer, FALSE);
  gtk_tree_view_column_set_attributes (col, renderer, "text", ESPM_DEVICE_INFO_VALUE, NULL);
...
static void
settings_quit (GtkWidget *widget, EsconfChannel *channel)
{
...
static void dialog_response_cb (GtkDialog *dialog, gint response, EsconfChannel *channel)
{
  switch(response)
  {
  case GTK_RESPONSE_HELP:
    expidus_dialog_show_help_with_version (NULL, "expidus1-power-manager", "start", NULL, ESPM_VERSION_SHORT);
    break;
  default:
...
static void
delete_event_cb (GtkWidget *plug, GdkEvent *ev, EsconfChannel *channel)
{
  settings_quit (plug, channel);
}

GtkWidget *
espm_settings_dialog_new (EsconfChannel *channel, gboolean auth_suspend,
                          gboolean auth_hibernate, gboolean can_suspend,
                          gboolean can_hibernate, gboolean can_shutdown,
                          ...
                          GtkApplication *gtk_app)
{
  ...
  ESPM_DEBUG ("auth_hibernate=%s auth_suspend=%s can_shutdown=%s can_suspend=%s can_hibernate=%s " \
              ...
              espm_bool_to_string (has_battery), espm_bool_to_string (auth_hibernate),
              espm_bool_to_string (can_shutdown), espm_bool_to_string (auth_suspend),
              espm_bool_to_string (can_suspend), espm_bool_to_string (can_hibernate),
              espm_bool_to_string (has_lcd_brightness), espm_bool_to_string (has_lid),
              espm_bool_to_string (has_sleep_button), espm_bool_to_string (has_hibernate_button),
              espm_bool_to_string (has_power_button), espm_bool_to_string (has_battery_button));

  xml = espm_builder_new_from_string (espm_settings_ui, &error);

  if ( G_UNLIKELY (error) )
  {
    expidus_dialog_show_error (NULL, error, "%s", _("Check your power manager installation"));
  ...
  val = esconf_channel_get_uint (channel, ESPM_PROPERTIES_PREFIX ON_BATTERY_BLANK, 1);
  gtk_range_set_value (GTK_RANGE (on_battery_display_blank), val);
  esconf_g_property_bind (channel, ESPM_PROPERTIES_PREFIX ON_BATTERY_BLANK,
  ...
  esconf_g_property_bind (channel, ESPM_PROPERTIES_PREFIX ON_AC_BLANK,
  ...
  esconf_g_property_bind (channel, ESPM_PROPERTIES_PREFIX HANDLE_BRIGHTNESS_KEYS,
                          G_TYPE_BOOLEAN, switch_widget, "active");

  switch_widget = GTK_WIDGET (gtk_builder_get_object (xml, "show-notifications"));
  esconf_g_property_bind (channel, ESPM_PROPERTIES_PREFIX GENERAL_NOTIFICATION_CFG,
                          G_TYPE_BOOLEAN, switch_widget, "active");

  switch_widget = GTK_WIDGET (gtk_builder_get_object (xml, "show-systray"));
  esconf_g_property_bind (channel, ESPM_PROPERTIES_PREFIX SHOW_TRAY_ICON_CFG,
                          G_TYPE_BOOLEAN, switch_widget, "active");

  dialog = GTK_WIDGET (gtk_builder_get_object (xml, "espm-settings-dialog"));
  nt = GTK_WIDGET (gtk_builder_get_object (xml, "main-notebook"));

  /* Set Gtk style */
  css_provider = gtk_css_provider_new ();
  gtk_css_provider_load_from_data (css_provider,
                                   ".expidus1-scale-label { padding-bottom: 0; }",
                                   -1, NULL);
  ...
  if ( has_battery )
  espm_settings_on_battery (channel,
                            auth_suspend,
                            auth_hibernate,
                            can_suspend,
                            can_hibernate,
  ...
  espm_settings_general (channel, auth_suspend, auth_hibernate, can_suspend, can_hibernate, can_shutdown,
                         has_sleep_button, has_hibernate_button, has_power_button, has_battery_button);

  espm_settings_advanced (channel, auth_suspend, auth_hibernate, can_suspend, can_hibernate, has_battery);

  /* Light Locker Integration */
  espm_settings_light_locker (channel, auth_suspend, auth_hibernate, can_suspend, can_hibernate);
  /* END Light Locker Integration */
...
void
espm_settings_show_device_id (gchar *device_id)
{
